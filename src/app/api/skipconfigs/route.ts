...
export async function GET(request: NextRequest) {
  try {
    ...
    if (source && id) {
      // 获取单个配置
      const config = await db.getSkipConfig(source, id);
      return NextResponse.json(config);
    } else {
      // 获取所有配置
      const configs = await db.getAllSkipConfigs();
...
export async function POST(request: NextRequest) {
  try {
    ...
    await db.setSkipConfig(source, id, skipConfig);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await db.deleteSkipConfig(source, id);
